    ...
    models.sequelize
      ...
      .then((authenticatedUser: { data: User }) => {
        ...
        if (user.data?.id && user.data.totpSecret !== "") {
          ...
          // The 'email' from the request body is directly reflected in the response.
          // A malicious user could inject HTML/JavaScript here.
          res
            .status(401)
            .send(`Invalid email or password for: ${req.body.email || ""}.`);
        }
      })
      .catch((error: Error) => {
        next(error);
